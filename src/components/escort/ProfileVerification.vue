...
const emit = defineEmits<{
  ...
  'start-verification': [profileId: string]
}>()
...
const idVerified = computed(() => props.profile.verification?.idVerified || false)
const photoVerified = computed(() => props.profile.verification?.photoVerified || false)
...
const submitVerification = async () => {
  ...
  try {
    ...
    formData.append('profileId', props.profile.id)
    ...
    await profileStore.submitVerification(props.profile.id, formData)
    
    clearInterval(progressInterval)
    uploadProgress.value = 100
    
    // Update profile verification status
    await profileStore.updateProfile(props.profile.id, {
      verification: {
...
<template>
  <div class="profile-verification">
    ...
    <!-- Verification Modal -->
    <div v-if="showVerificationModal" class="verification-modal" @click.self="closeModal">
